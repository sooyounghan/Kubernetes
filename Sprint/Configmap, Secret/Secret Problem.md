...
2. Secret은 Configmap과 달리 type이라는 속성 존재 : 기본적으로 넣지 않으면 opaque라는 값으로 설정
   - Opaque(투명)이라는 값이 들어가 Configmap을 써도 되는 모든 상황에 조금 중요한 데이터라는 생각이 되면 타입의 Secrtet으로 대체 가능 : 즉, Configmap과 동일
...
3. 데이터 암호화 방법
   ...
   - 암호화를 관리해주는 Third-Party 도구 사용 : HashiCorp의 Valut가 대포ㅛ적
