...
6. ArgoCD가 Kubernetes에 설치됐을 때 아키텍쳐
   ...
   - ArgoCD를 설치하면 수 많은 컴포넌트들이 설치
     ...
     + Repo Server : GitHub (App에 대해 배포할 YAML 파일들이 존재)가 존재하며, Git에 연결해서 YAML 파일들을 가져오며, 이를 통해 Kubernetes에 배포할 Manifest 파일을 만들어 놓는 역할을 함
     + Application Controller : Kubernetes의 리소스를 모니터링하면서 Git에 받은 내용과 다른게 있는지 비교하며, 내용이 다르면 Git에 있는 내용으로 배포 진행
     + Kube API : Kubernetes로 리소스 생성 명령을 전송하는 역할
