...
2. 사용 예 : Canary Upgrade
   - V1의 App들이 구동되서 서비스가 되고 있는 상태에서 테스트 할 버전 V2의 App을 구도시키고 Ingress를 만들어서 두 버전의 서비스를 ㅇ녀결하면, Ingress를 통해 접근을 했을 때, 10%의 일부 트래픽만 V2로 가도록 설정 가능
...
4. Ingress의 기능
   - Service LoadBalancing
     + 각 업무 별로 Pod와 Service를 생성하며, 사전에 Ngnix가 설치되어있고, 이 Pod의 외부에서 연결이 되도록 Node Port 서비스도 연결되어 있으므로 마스터의 호스트 IP인 ```192.168.0.30:30431```로 접근하면 Pod의 80번 APort로 트래픽이 전송
